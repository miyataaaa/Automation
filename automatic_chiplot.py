...
def Do_chimacro_on_fiels(path1, path2, input_nm):
...
def extract_chi_z(fname, **kwargs):
...
def compile_specific_mn(**kwargs):
    ...
    for curDir, dirs, files in os.walk(dirpath):
        for file in files:
            if file.startswith("Chiplot"):
                ...
                if i == 0:
                    df = extract_chi_z(file, **kwargs)
                else:
                    df = pd.concat([df, extract_chi_z(file, **kwargs)], axis=1)
            i+=1
            
    return df

def save_to_newfile(**kwargs):
    
    dirpath = kwargs["dirpath"]
    m = kwargs["m"]
    n = kwargs["n"]
    df = compile_specific_mn(**kwargs)
...
def value_from_compiled_file(fname, **kwargs):
...
def compiled_chiplot(df_sheet, coefs, **kwargs):
